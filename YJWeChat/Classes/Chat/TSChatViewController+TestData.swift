...
extension TSChatViewController {
  ...
  func updateTableWithNewRowCount(_ count: Int) {
    var contentOffset = listTableView.contentOffset
    ...
    for i in 0 ..< count {
      ...
      heightForNewRows += tableView(listTableView, heightForRowAt: indexPath)
    }
    contentOffset.y += heightForNewRows
    
    listTableView.insertRows(at: indexPaths, with: UITableViewRowAnimation.none)
    listTableView.endUpdates()
    UIView.setAnimationsEnabled(true)
    listTableView.setContentOffset(contentOffset, animated: false)
  }
}
